#include "Pingouin.hpp"
#include <algorithm>
#include <iostream>
#include <iomanip>
#include <limits>
#include <ostream>
#include <limits>

std::vector<std::weak_ptr<Pingouin>> Pingouin::colonie;
std::vector<std::string> Pingouin::lieux;
std::map<std::string,int> Pingouin::competences;

void Pingouin::nettoyerColonie() {
    auto it = std::remove_if(colonie.begin(), colonie.end(), [](const std::weak_ptr<Pingouin> &w) {
        return w.expired();
    });
    colonie.erase(it, colonie.end());
}

std::shared_ptr<Pingouin> Pingouin::create(const std::string &nom, double vNage, double vMarche, double vGlisse) {
    auto p = std::make_shared<Pingouin>(nom, vNage, vMarche, vGlisse);
    colonie.push_back(p);
    nettoyerColonie();
    trierColonie();
    return p;
}

Pingouin::~Pingouin() {}

void Pingouin::nage() {
    std::cout << "[Pingouin] " << nom_ << " nage a " << getVitesseNage() << " m/s" << std::endl;
}

void Pingouin::marche() {
    std::cout << "[Pingouin] " << nom_ << " marche a " << getVitesseMarche() << " m/s" << std::endl;
}

void Pingouin::sePresenter() const {
    std::cout << "Bonjour, je suis " << nom_ << ", je nage a " << vitesseNage_ << " m/s, marche a " << vitesseMarche_
              << " m/s et glisse a " << vitesseGlisse_ << " m/s" << std::endl;
}

double Pingouin::tempsParcours() const {
    using std::numeric_limits;
    if (vitesseGlisse_ <= 0.0 || vitesseMarche_ <= 0.0 || vitesseNage_ <= 0.0) return numeric_limits<double>::infinity();
    double t = 0.0;
    t += 15.0 / vitesseGlisse_;
    t += 20.0 / vitesseMarche_;
    t += 50.0 / vitesseNage_;
    t += 15.0 / vitesseMarche_;
    return t;
}

void Pingouin::afficherTempsTous() {
    nettoyerColonie();
    std::cout << std::fixed << std::setprecision(2);
    for (auto it = colonie.begin(); it != colonie.end(); ) {
        if (auto sp = it->lock()) {
            double t = sp->tempsParcours();
            if (std::isfinite(t)) std::cout << sp->nom_ << " : " << t << " s\n";
            else std::cout << sp->nom_ << " : N/A\n";
            ++it;
        } else {
            it = colonie.erase(it);
        }
    }
}

void Pingouin::trierColonie() {
    nettoyerColonie();
    std::sort(colonie.begin(), colonie.end(), [](const std::weak_ptr<Pingouin> &aW, const std::weak_ptr<Pingouin> &bW) {
        auto a = aW.lock();
        auto b = bW.lock();
        if (!a && !b) return false;
        if (!a) return false;
        if (!b) return true;
        double ta = a->tempsParcours();
        double tb = b->tempsParcours();
        bool aFin = std::isfinite(ta);
        bool bFin = std::isfinite(tb);
        if (aFin && bFin) {
            if (ta != tb) return ta < tb;
            return a->nom_ < b->nom_;
        }
        if (aFin) return true;
        if (bFin) return false;
        return a->nom_ < b->nom_;
    });
}

// Job06: lieux management
void Pingouin::ajouterLieu(const std::string &lieu) {
    if (std::find(lieux.begin(), lieux.end(), lieu) == lieux.end()) lieux.push_back(lieu);
}

void Pingouin::retirerLieu(const std::string &lieu) {
    lieux.erase(std::remove(lieux.begin(), lieux.end(), lieu), lieux.end());
}

void Pingouin::afficherLieux() {
    if (lieux.empty()) {
        std::cout << "Aucun lieu de rencontre defini." << std::endl;
        return;
    }
    std::cout << "Lieux de rencontre:" << std::endl;
    for (const auto &l : lieux) std::cout << "- " << l << std::endl;
}

// Job07: competences management
void Pingouin::ajouterCompetence(const std::string &nom, int niveau) {
    if (competences.find(nom) == competences.end()) competences[nom] = niveau;
}

void Pingouin::modifierCompetence(const std::string &nom, int niveau) {
    auto it = competences.find(nom);
    if (it != competences.end()) it->second = niveau;
}

void Pingouin::retirerCompetence(const std::string &nom) {
    competences.erase(nom);
}

void Pingouin::afficherCompetences() {
    if (competences.empty()) {
        std::cout << "Aucune competence definie." << std::endl;
        return;
    }
    std::cout << "Competences:" << std::endl;
    for (const auto &p : competences) std::cout << "- " << p.first << " : " << p.second << std::endl;
}

// Job08: friends management
bool Pingouin::ajouterAmi(const std::shared_ptr<Pingouin> &ami) {
    if (!ami) return false;
    if (ami.get() == this) return false; // refuse self
    // insert weak_ptr; owner_less ensures ordering
    auto result = amis_.insert(ami);
    // result is pair<iterator,bool>
    return result.second;
}

bool Pingouin::retirerAmi(const std::shared_ptr<Pingouin> &ami) {
    if (!ami) return false;
    auto it = amis_.find(ami);
    if (it == amis_.end()) return false;
    amis_.erase(it);
    return true;
}

void Pingouin::nettoyerAmis() {
    for (auto it = amis_.begin(); it != amis_.end(); ) {
        if (it->expired()) it = amis_.erase(it);
        else ++it;
    }
}

// Journal d'aventures
bool Pingouin::ajouterAventure(const std::string &date, const std::string &description) {
    auto [it, inserted] = journalAventures_.emplace(date, description);
    return inserted; // false if date already present
}

bool Pingouin::modifierAventure(const std::string &date, const std::string &description) {
    auto it = journalAventures_.find(date);
    if (it == journalAventures_.end()) return false;
    it->second = description;
    return true;
}

bool Pingouin::retirerAventure(const std::string &date) {
    return journalAventures_.erase(date) > 0;
}

void Pingouin::afficherAventures() const {
    if (journalAventures_.empty()) {
        std::cout << "Aucune aventure enregistrée.\n";
        return;
    }
    std::cout << "Journal d'aventures de " << nom_ << ":\n";
    for (const auto &p : journalAventures_) {
        std::cout << p.first << " : " << p.second << "\n";
    }
}

// Job10: competition times (milliseconds)
bool Pingouin::ajouterTempsCompetitionMs(long long ms) {
    tempsCompetitionsMs_.insert(ms);
    return true;
}

bool Pingouin::retirerUneOccurrenceTempsMs(long long ms) {
    auto it = tempsCompetitionsMs_.find(ms);
    if (it == tempsCompetitionsMs_.end()) return false;
    tempsCompetitionsMs_.erase(it);
    return true;
}

size_t Pingouin::compterOccurencesTempsMs(long long ms) const {
    return tempsCompetitionsMs_.count(ms);
}

std::vector<long long> Pingouin::obtenirTempsTriesMs() const {
    std::vector<long long> out;
    out.reserve(tempsCompetitionsMs_.size());
    for (auto v : tempsCompetitionsMs_) out.push_back(v);
    return out;
}

void Pingouin::afficherTempsCompetitionsMs(std::ostream &os) const {
    if (tempsCompetitionsMs_.empty()) {
        os << "Aucun temps de compétition enregistré.\n";
        return;
    }
    os << "Temps de compétition (triés) pour " << nom_ << ":\n";
    os << std::fixed << std::setprecision(3);
    for (auto ms : tempsCompetitionsMs_) {
        double s = ms / 1000.0;
        os << "- " << s << " s\n";
    }
}

std::vector<std::shared_ptr<Pingouin>> Pingouin::listerAmis() const {
    std::vector<std::shared_ptr<Pingouin>> res;
    // note: amis_ is mutable? We didn't declare mutable; but listerAmis is const.
    // We'll perform a const_cast to call nettoyerAmis (simple for demo).
    const_cast<Pingouin*>(this)->nettoyerAmis();
    for (const auto &w : amis_) {
        if (auto sp = w.lock()) res.push_back(sp);
    }
    return res;
}
