...
private:
    ...
    // Times of competitions in milliseconds (sorted, duplicates allowed)
    std::multiset<long long> tempsCompetitionsMs_;
    std::string nom_;
...
public:
    ...
    // Job10: competition times
    bool ajouterTempsCompetitionMs(long long ms);
    bool retirerUneOccurrenceTempsMs(long long ms);
    size_t compterOccurencesTempsMs(long long ms) const;
    void afficherTempsCompetitionsMs(std::ostream& os = std::cout) const;
    std::vector<long long> obtenirTempsTriesMs() const;
